...
.PHONY: init capabilities check privacy-seal privacy-preflight preview approve copy chapters audio faq shorts clean-debug validate final release test

init:
	test -n "$(NAME)"
	$(PYTHON) scripts/meetup-video.py --project "projects/$(NAME)/project.json" init --name "$(NAME)" $(if $(EVENT_URL),--event-url "$(EVENT_URL)") $(if $(TEMPLATE),--template "$(TEMPLATE)")
