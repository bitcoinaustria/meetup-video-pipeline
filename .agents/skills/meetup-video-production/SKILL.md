...
5. Run `make final PROJECT=<project>`. The command rebuilds audio and FAQ decisions, renders the configured production resolution, validates it, and only then replaces the previous final atomically.
6. Run `make shorts PROJECT=<project>` when the Shorts manifest contains approved clips. Subtitles remain separate SRT files.
